...
class AnalogTernaryLinear(AnalogLinear):
    ...
    def ternary_weight_cpu(self):
        ...
        return torch.where(
            w > self.threshold,
            torch.ones_like(w),
            torch.where(w < -self.threshold, -torch.ones_like(w), torch.zeros_like(w)),
        )

    def form_matrix(self):
        weight_ternary = self.ternary_weight_cpu()

        if self.analog_bias:
            return from_dlpack(
                self.core.form_matrix(weight_ternary, self.bias.detach().cpu())
            )
        else:
            return from_dlpack(self.core.form_matrix(weight_ternary))

    def forward(self, x):
        return AnalogTernaryLinearGrad.apply(
            x,
            self.weight,
            self.bias,
            self.core,
    ...
    def from_torch(cls, layer, params, bias_rows=0):
        ...
        threshold = getattr(layer, "threshold", 0.05)
        width = getattr(layer, "width", 0.7)
...
class AnalogTernaryLinearGrad(Function):
    ...
    def forward(
        ...
        if training:
            w_core, b_core = core.get_core_weights()
            w_core = from_dlpack(w_core)

            if not analog_bias:
                b_core = bias
            else:
                b_core = from_dlpack(b_core)

            ctx.save_for_backward(x, weight, w_core, b_core)
    ...
    @staticmethod
    def backward(ctx, grad_output):
        x, shadow_weight, core_weight, bias = ctx.saved_tensors
        ...
        # Gradient to input uses physical/core weight
        if ctx.needs_input_grad[0]:
            grad_x = grad_output.matmul(core_weight)

        # Normal linear gradient
        if ctx.needs_input_grad[1]:
            if x.ndim == 1:
                grad_weight = kron(grad_output, x).reshape(core_weight.shape)
            elif x.ndim == 2:
                grad_weight = grad_output.t().matmul(x)
            else:
                x_ = x.flatten(0, x.ndim - 2)
            ...
            # Smooth threshold-window surrogate
            dist_pos = (shadow_weight - threshold).abs()
            dist_neg = (shadow_weight + threshold).abs()
            ...
            grad_weight = grad_weight * grad_surrogate
